...
def make_train_valid_test_directories(path_to_data_folder,
                                      class_names_folders,
                                      valid_percent=0.18,
                                      test_percent=0.18):
    """
    ...
    train_test_valid_split(class_names_folders, path_to_data_folder,
                           valid_percent, test_percent)


def train_test_valid_split(class_names_folders, path_to_data_folder,
                           valid_percent, test_percent):
    for name in class_names_folders:
        ...
        test_img_indices = set(
            np.random.randint(0, len(listImagesDir), num_test_imgs))
        valid_img_indices = get_valid_img_indices(test_img_indices,
                                                  num_valid_imgs,
                                                  len(listImagesDir))
        ...
        for img_index in range(len(listImagesDir)):
            ...
            if img_index in test_img_indices:
                copyfile(
                    orig_path_img,
                    '%s/test/%s/%s' % (path_to_data_folder, name, curr_img))
            elif img_index in valid_img_indices:
                copyfile(
                    orig_path_img,
                    '%s/valid/%s/%s' % (path_to_data_folder, name, curr_img))
            else:
                copyfile(
                    orig_path_img,
                    '%s/train/%s/%s' % (path_to_data_folder, name, curr_img))
...
if __name__ == "__main__":
    ...
    argparseObj.add_argument('--class_names_folders', nargs="+", type=str)

    argparseObj.add_argument('--path_to_data_folder',
                             type=str,
                             default=os.path.abspath('') + '/data')

    argparseObj.add_argument('--valid_percent', type=float, default=0.18)

    argparseObj.add_argument('--test_percent', type=float, default=0.18)

    args = argparseObj.parse_args()
    make_train_valid_test_directories(args.path_to_data_folder,
                                      args.class_names_folders,
